...
path{1} = 'C:\Cannula Microscope\20230821_100x100_100umPSF\';  zInd(1) = 0; % don't forget to change name at botom if needed
...
%           imgHDR = imgHDR./65540;
            % shift for x and y
            % if (sfX(z+1) ~= 0 || sfY(z+1) ~= 0) % could get rid of shift, but there isn't any so no need to use it
            %     refPro = imref2d(size(imgHDR));
            %     tform = affine2d([1 0 0; 0 1 0; sfX(z+1) sfY(z+1) 1]);
            %     imgHDR = imwarp(imgHDR,tform,'OutputView',refPro,'FillValues',0);
            % end
            % 
            % if scaleFactor ~= 1
            %imgHDR = imresize(imgHDR,scaleFactor);
            % Finding the min and max value within the mask
            %min_value_within_mask = min(imgHDR(mask));
            %max_value_within_mask = max(imgHDR(mask));
...
%         for x = 0:psfSize-1
%             filename = num2str(z);        
%             load([path{1},filename]);
% 
%             psf(:,:,y+1,x+1,z+1) = (imgHDR - DC);
%         end
%     end
% end
...
%% Get Calibration parallelized
% delete(gcp('nocreate'));
% Initializing the PSF variable
psf = zeros(imageSize, imageSize, psfSize, psfSize, zSize, 'single');

parpool(4); % Start the parallel pool

parfor z = 0:zSize-1
    fprintf('-------------------------z = %d------------------------\n', z);
    local_psf = zeros(imageSize, imageSize, psfSize, psfSize, 'single');
    
    for y = 0:psfSize-1
        fprintf('y = %d\n', y);
        for x = 0:psfSize-1
            filename = ['x', num2str(x), '_y', num2str(y), '_z', num2str(zInd(z+1))];
            data = load([path{z+1}, filename]);
            ...
            % Resize the image as per scaleFactor
            ...
            local_psf(:,:,y+1,x+1) = (imgHDR - DC); % 5D PSF, img_x, img_y, X, Y, Z
        end
    end
    
    psf(:,:,:,:,z+1) = local_psf; % Assigning the local slice to the main variable
end
...
for z = 0:zSize-1
    for x = 0:psfSize-1
        for y = 0:psfSize-1
            if (normArray(y+1,x+1) < 0.13)  %0.1 if the mean pixel value is less than 0.13, it is set to background
