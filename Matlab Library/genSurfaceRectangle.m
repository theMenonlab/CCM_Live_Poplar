function Surface = genSurfaceRectangle(Support)

%clear, clc
% genSurface2 is generates a surface with only the n closest points
...
Surface = zeros(3,length(Ymat)*length(Xmat));
% initialize surface
...
for m = 1:length(Surface)
    for p = 1:length(Support)
        L(4,p) = sqrt((Support(1,p) - Surface(1,m))^2 + (Support(2,p) - Surface(2,m))^2);
    end

    [temp, order] = sort(L(4,:));
    L_sorted = L(:,order);
    L_sorted = L_sorted(:, 1:3); % calculates weights from n closest points
    W = zeros(length(L_sorted),1);
    
    for p = 1:length(L_sorted(1))
        W(p) = 1/(L_sorted(4,p)^Power); % Weight by distance from point
        Surface(3,m) = Surface(3,m) + L_sorted(3,p) * W(p); % Sum of z value times weights
    end
...
% while ~isempty(rNAN)
%     if cNAN(1) == 1
%         Surface(rNAN(1), cNAN(1)) = 0; % or any default value other than NaN
%     else
...
% figure()
% plot(Surface(1,200:230), Surface(3, 200:230))
end