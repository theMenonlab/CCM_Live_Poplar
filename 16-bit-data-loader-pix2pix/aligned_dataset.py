...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
    	...
    	AB = Image.open(AB_path) # .convert("I")  # convert to 16-bit
    	# split AB image into A and B
    	w, h = AB.size
    	w2 = int(w / 2)
    	A = AB.crop((0, 0, w2, h))# .convert("I")
    	B = AB.crop((w2, 0, w, h))# .convert("I")

    	# apply the same transform to both A and B
    	transform_params = get_params(self.opt, A.size)
    	A_transform = get_transform(transform_params, grayscale=(self.input_nc == 1))
    	B_transform = get_transform(transform_params, grayscale=(self.output_nc == 1))
